...
void identify_instruction_and_operand(conf_object_t *obj, conf_object_t *cpu, logical_address_t address)
{
    ...
    if (strncmp(entry->disassembled_text, "mov", 3) == 0 ||
        ...
        strncmp(entry->disassembled_text, "div", 3) == 0 ||
        strncmp(entry->disassembled_text, "xor", 3) == 0 ||
        strncmp(entry->disassembled_text, "shr", 3) == 0)
    {
        for (int i = 0; i < conn->total_reservation_stations; ++i)
        {
...
pipeline_algorithm(conf_object_t *obj, conf_object_t *cpu, void *user_data)
{
    ...
    {
        end_time = clock();
        double time_taken = ((double)end_time - (double)start_time) / (double)CLOCKS_PER_SEC;

        SIM_LOG_INFO(1, obj, 0, "-------------------- Current CPU Cycle: %d --------------------", conn->cycles + 1);
        SIM_LOG_INFO(1, obj, 0, "Program finished! in %f seconds :D", time_taken);
    ...
    { // In case you want to print it all the time use log level 4
                        ...
                        void *data)
{
    ...
    // get the logical address of the instruction
    logical_address_t la = conn->iq_iface->logical_address(cpu, i_handle);

    // get the bytes
    cpu_bytes_t bytes = conn->iq_iface->get_instruction_bytes(cpu, i_handle);

    // Makes a data object
    attr_value_t op_code = SIM_make_attr_data(bytes.size, bytes.data);
